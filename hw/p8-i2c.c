...
static int p8_i2c_check_initial_status(struct p8_i2c_master_port *port)
{
	...
	if (status & I2C_STAT_ANY_ERR) {
		...
		master->recov_time = mftb() + msecs_to_tb(5);
				...
				struct i2c_request *req)
{
	...
	DBG("Starting req @%p %d len=%d addr=%02x (offset=%x)\n", req,
	...
	request->timeout = mftb() + (tbytes * msecs_to_tb(1));
...
static void p8_i2c_check_timeout(struct p8_i2c_master *master)
{
	struct p8_i2c_master_port *port;
	struct p8_i2c_request *request;
	struct i2c_request *req;
...
static void p8_i2c_poll(void *data)
{
	...
	if (master->state == state_idle)
		return;
	if (tb_compare(now, master->next_poll) == TB_ABEFOREB)
		return;

	lock(&master->lock);
	master->next_poll = now + usecs_to_tb(10);
	p8_i2c_check_status(master);
	if (master->state == state_recovery) {
		if (tb_compare(now, master->recov_time) != TB_ABEFOREB)
			master->state = state_idle;
	} else if (master->state != state_idle)
		p8_i2c_check_timeout(master);
	p8_i2c_check_work(master);
	unlock(&master->lock);
}
...
static void p8_i2c_init_one(struct dt_node *i2cm, enum p8_i2c_master_type type)
{
	...
	opal_add_poller(p8_i2c_poll, master);
}
